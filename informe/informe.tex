...
Este documento presenta la metodología, con la teoría necesaria en cada
paso, y el análisis de los resultados. La implementación completa, los experimentos
...
Esta sección describe el problema, los algoritmos y el diseño de los
experimentos. Los conceptos teóricos y las fórmulas se presentan en el
punto donde se usan, junto a la decisión de diseño que sostienen.

\subsection{Enfoque: búsqueda local}
...
(entre 16 y 28), calculada con la búsqueda en anchura descrita en la
formulación.

\subsection{Algoritmos}

\subsubsection{Ascenso de colinas: más pronunciado y estocástico}
...
consecuencias futuras. Se implementaron dos variantes:
...
Las dos se detienen en el primer mínimo local. En el 8-puzzle, como
$\Delta h = \pm 1$, un paso de ascenso siempre baja $h$ en exactamente
1, así que el camino de un ascenso tiene como máximo $h(s_0)$
movimientos y nunca retrocede.

\subsubsection{Reinicios por perturbación y elección de \texorpdfstring{$K$}{K}}

El ascenso con reinicios aleatorios (\emph{random restart}) repite el
ascenso desde distintos puntos de partida hasta encontrar la meta o
agotar un presupuesto. Si cada intento tiene probabilidad $p$ de
éxito, el número esperado de intentos es $1/p$, y el algoritmo es
completo con probabilidad que tiende a 1 \cite{russell2021}. Como no
tiene un criterio de parada propio, necesita un presupuesto de
evaluaciones.
...
diámetro del espacio de estados (31, ver la formulación). Con $K < 31$
la caminata sigue siendo una modificación local del tablero asignado.
Con $K \ge 31$ ya tiene alcance para llegar a cualquier tablero
resoluble, y el reinicio se parece cada vez más a generar un tablero al
azar. $K = 20$ es el mayor valor redondo por debajo de ese umbral. El
barrido $K \in \{10, 20, 50, 100, 200\}$ se reporta aparte como
análisis del efecto de $K$, no como criterio de selección.
...
La temperatura baja con enfriamiento geométrico: se hacen $L$
iteraciones con cada temperatura y después se actualiza
$T_{k+1} = \alpha \, T_k$, desde $T_0$ hasta $T \le T_{\min}$. La
ejecución también se detiene si $h$ llega a 0. Los parámetros son la
temperatura inicial $T_0$, el factor de enfriamiento
$\alpha \in (0, 1)$, las iteraciones por nivel $L$ y la temperatura
mínima $T_{\min}$. El número de niveles de temperatura es
\begin{equation}
...
elegir $T_0$ y $\alpha$ es un compromiso entre calidad y costo; por eso
se eligieron con un barrido (Sección \ref{sec:barrido_temple}).

Los otros dos parámetros quedaron fijos en $L = 100$ iteraciones por
nivel y $T_{\min} = 0.05$. Con $T = 0.05$, aceptar un empeoramiento
tiene probabilidad $e^{-20}$: el temple ya está congelado. Se probó
$T_{\min} = 0.01$ (el valor de un cuaderno exploratorio previo): temple
de la actividad sin tope dio exactamente la misma tasa de éxito
(70.3\%), pero gastó 11\,395 evaluaciones en lugar de 9823, porque solo
alarga la cola del enfriamiento. Bajarlo habría inflado el presupuesto
compartido sin cambiar el resultado.
